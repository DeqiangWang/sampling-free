...
from maskrcnn_benchmark.modeling.sampling_free import ImbalancedDecider
...
def make_retinanet_postprocessor(config, rpn_box_coder, is_train):
    ...
    box_selector = RetinaNetPostProcessor(
        ...
        imbalanced_decider=ImbalancedDecider(config, arch="RetinaNet", num_classes=config.MODEL.ROI_BOX_HEAD.NUM_CLASSES) if config.MODEL.SAMPLING_FREE_ON else None
    )
